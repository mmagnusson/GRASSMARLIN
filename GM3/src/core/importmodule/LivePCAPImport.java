...
public class LivePCAPImport extends ImportItem {
    ...
    public LivePCAPImport(Path dumpPath, PcapNetworkInterface device, List<Fingerprint> fingerprints, Runnable fnOnNewPacket) {
        super(dumpPath, fingerprints);
        ImportProcessors.ProcessorWrapper wrapper = ImportProcessors.processorForPath(dumpPath);
        if (wrapper == null) {
            throw new IllegalStateException("No import processor registered for " + dumpPath.getFileName() + ". PCAP support may not be initialized.");
        }
        source = ImportProcessors.newItem(wrapper.getProcessor(), dumpPath, fingerprints);
